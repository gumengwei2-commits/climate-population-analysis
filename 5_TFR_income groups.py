...
def train_shap_for_group(df_group, group_name, table_dir, fig_dir):
    ...
    performance_df.to_csv(os.path.join(table_dir, f"TFR_XGB_performance_{group_name.replace(' ','_')}.csv"), 
    ...
    shap_df.to_csv(os.path.join(table_dir, f'TFR_shap_values_{group_name.replace(" ","_")}.csv'), index=False)

    # 保存 SHAP summary TIFF
    plt.figure(figsize=(6,6))
    shap.summary_plot(shap_values, X, show=False, plot_type='dot', max_display=10)
    plt.tight_layout()
    plt.savefig(os.path.join(fig_dir, f'TFR_shap_summary_{group_name.replace(" ","_")}.tiff'), dpi=300)
    plt.close()
...
saved_images = [os.path.join(fig_dir, f'TFR_shap_summary_{g.replace(" ","_")}.tiff') for g in shap_data.keys()]
...
# 保存最终 TIFF
final_im.save(os.path.join(fig_dir, 'TFR_shap_summary_income_comparison.tiff'), dpi=(300,300))
