...
def train_group_XGB(df_group, group_name):
    ...
    performance_df.to_csv(os.path.join(table_dir, f"TFR_XGB_performance_{group_name}.csv"), index=False, encoding='utf-8-sig')

    # ---------------------- SHAP ----------------------
    explainer = shap.TreeExplainer(model)
    shap_values = explainer.shap_values(X)
    shap_df = pd.DataFrame(shap_values, columns=X.columns)
    shap_df.to_csv(os.path.join(table_dir, f'TFR_shap_values_{group_name}.csv'), index=False)

    plt.figure(figsize=(8,6))
    shap.summary_plot(shap_values, X, show=False)
    plt.tight_layout()
    plt.savefig(os.path.join(fig_dir, f'TFR_shap_summary_{group_name}.tiff'), dpi=300)
    plt.close()
...
if all(k in shap_data for k in ['mainland','island']):
    ...
    plt.savefig(os.path.join(fig_dir, 'TFR_shap_summary_comparison.tiff'), dpi=300)
    plt.close()
