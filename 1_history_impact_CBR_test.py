...
def plot_shap_group(X, shap_values, y, features, title, output_file):
    ...
    for i, feature in enumerate(top_features):
        ...
        cbar.outline.set_visible(False)
        # ✅ 保留标题
        cbar.ax.set_title("CBR", fontsize=14)
        cbar.ax.tick_params(
            axis='y',
...
excel_path = os.path.join(table_dir, "XGB_Model_Performance_Report.xlsx")
...
shap_path = os.path.join(table_dir, "SHAP_results_xgb.csv")
...
plot_shap_group(X_test, shap_values, y_test, climate_vars, "Mean SHAP Value (Climate)",
                os.path.join(fig_dir, "CBR_SHAP_Climate.tiff"))
plot_shap_group(X_test, shap_values, y_test, control_vars, "Mean SHAP Value (Socioeconomic)",
                os.path.join(fig_dir, "CBR_SHAP_Socioeconomic.tiff"))

# ---------------------- 测试集预测效果可视化 ----------------------
plt.figure(figsize=(6,6))
plt.scatter(y_test, y_test_pred, alpha=0.6)
plt.plot([y_test.min(), y_test.max()], [y_test.min(), y_test.max()], 'r--')
plt.xlabel("Actual Birth Rate (CBR)")
plt.ylabel("Predicted Birth Rate")
plt.title(f"XGBoost Test Set Prediction\nR² = {r2_test:.3f}")
plt.grid(True)
plt.tight_layout()
plt.savefig(os.path.join(fig_dir, "XGB_R2_Test.tiff"), dpi=300)
plt.show()
